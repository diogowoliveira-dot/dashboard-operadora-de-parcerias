...
// ═══════════════════════════════════════════
// PLAYBOOK TAB — tracking de acessos (master/admin)
// ═══════════════════════════════════════════
const apiPlaybook = {
  materiais: () => fetch('/api/playbook?action=materiais').then(r => r.json()),
  tracking: () => fetch('/api/playbook?action=tracking').then(r => r.json()),
  userDetail: (id) => fetch(`/api/playbook?action=user-detail&id=${id}`).then(r => r.json()),
  createMaterial: (data) => fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'create-material', ...data }) }).then(r => r.json()),
  updateMaterial: (data) => fetch('/api/playbook', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => r.json()),
  deleteMaterial: (id) => fetch(`/api/playbook?id=${id}`, { method: 'DELETE' }).then(r => r.json()),
  logAccess: (material_id) => fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'log-access', material_id }) }).then(r => r.json()),
};
...
function PlaybookTab({ isMaster, isGestor, authUser }) {
  const canManageContent = isMaster; // só master cria/edita/deleta materiais
  const canManageTrilhas = isMaster || isGestor; // master e gestores criam trilhas
  ...
  const [sortBy, setSortBy] = useState('acessos');
  const [viewMode, setViewMode] = useState('cards');

  // Meu progresso (user regular)
  ...
  const [trilhaMatIds, setTrilhaMatIds] = useState([]);
  const [trilhaUserIds, setTrilhaUserIds] = useState([]);
  const [trilhaSaving, setTrilhaSaving] = useState(false);
  const [selectedTrilha, setSelectedTrilha] = useState(null);
  const [trilhaDetail, setTrilhaDetail] = useState(null);
  ...
  const [matTipo, setMatTipo] = useState('documento');
  const [matUrl, setMatUrl] = useState('');
  const [matOrdem, setMatOrdem] = useState(0);
  const [matSaving, setMatSaving] = useState(false);
  const [matSortBy, setMatSortBy] = useState('ordem');
  ...
  const loadUserDetail = useCallback(async (userId) => {
    ...
    setShowFaltam(false);
    const data = await apiPlaybook.userDetail(userId);
    setUserDetail(data);
  ...
  const resetForm = () => {
    setEditingMat(null); setMatTitulo(''); setMatDesc('');
    setMatTipo('documento'); setMatUrl(''); setMatOrdem(0); setShowForm(false);
  };

  const openEditForm = (m) => {
    setEditingMat(m.id); setMatTitulo(m.titulo); setMatDesc(m.descricao || '');
    setMatTipo(m.tipo); setMatUrl(m.url || ''); setMatOrdem(m.ordem); setShowForm(true);
  };

  const saveMaterial = async () => {
    if (!matTitulo.trim()) return;
    setMatSaving(true);
    if (editingMat) {
      await apiPlaybook.updateMaterial({ id: editingMat, titulo: matTitulo, descricao: matDesc, tipo: matTipo, url: matUrl, ordem: matOrdem });
    } else {
      await apiPlaybook.createMaterial({ titulo: matTitulo, descricao: matDesc, tipo: matTipo, url: matUrl, ordem: matOrdem });
    }
    setMatSaving(false);
    resetForm();
    loadData();
  ...
  const deleteMaterial = async (id) => {
    if (!confirm('Desativar este material? Ele não será mais visível.')) return;
    await apiPlaybook.deleteMaterial(id);
    loadData();
  };

  // Trilha functions
  const resetTrilhaForm = () => {
    setEditingTrilha(null); setTrilhaNome(''); setTrilhaDesc('');
    setTrilhaMatIds([]); setTrilhaUserIds([]); setShowTrilhaForm(false);
  };

  const saveTrilha = async () => {
    if (!trilhaNome.trim()) return;
    setTrilhaSaving(true);
    if (editingTrilha) {
      await fetch('/api/playbook', { method: 'PATCH', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target: 'trilha', id: editingTrilha, nome: trilhaNome, descricao: trilhaDesc }) });
    } else {
      await fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'create-trilha', nome: trilhaNome, descricao: trilhaDesc, material_ids: trilhaMatIds, user_ids: trilhaUserIds }) });
    }
    setTrilhaSaving(false);
    resetTrilhaForm();
    loadData();
  };
  ...
  const deleteTrilha = async (id) => {
    ...
    await fetch(`/api/playbook?target=trilha&id=${id}`, { method: 'DELETE' });
    loadData();
  };

  const loadTrilhaDetail = async (id) => {
    const data = await fetch(`/api/playbook?action=trilha-detail&id=${id}`).then(r => r.json());
    setTrilhaDetail(data);
  };

  const addMatToTrilha = async (trilhaId, matId) => {
    await fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'trilha-add-materiais', trilha_id: trilhaId, material_ids: [matId] }) });
    loadTrilhaDetail(trilhaId);
  };

  const removeMatFromTrilha = async (trilhaId, matId) => {
    await fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'trilha-remove-material', trilha_id: trilhaId, material_id: matId }) });
    loadTrilhaDetail(trilhaId);
  };

  const addUserToTrilha = async (trilhaId, uid) => {
    await fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'trilha-add-usuarios', trilha_id: trilhaId, user_ids: [uid] }) });
    loadTrilhaDetail(trilhaId);
  };

  const removeUserFromTrilha = async (trilhaId, uid) => {
    await fetch('/api/playbook', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'trilha-remove-usuario', trilha_id: trilhaId, user_id: uid }) });
    loadTrilhaDetail(trilhaId);
  };

  // Sort users
  const sortedUsers = useMemo(() => {
    if (!tracking?.users) return [];
    const u = [...tracking.users];
    if (sortBy === 'acessos') u.sort((a, b) => b.total_acessos - a.total_acessos);
    else if (sortBy === 'nome') u.sort((a, b) => (a.name || a.email).localeCompare(b.name || b.email));
    else if (sortBy === 'faltam') u.sort((a, b) => (tracking.total_materiais - a.materiais_acessados) - (tracking.total_materiais - b.materiais_acessados));
    return u;
  }, [tracking, sortBy]);
  ...
  const sortedMateriais = useMemo(() => {
    const m = [...materiais];
    if (matSortBy === 'ordem') m.sort((a, b) => a.ordem - b.ordem || new Date(a.created_at) - new Date(b.created_at));
    else if (matSortBy === 'data') m.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    else if (matSortBy === 'titulo') m.sort((a, b) => a.titulo.localeCompare(b.titulo));
    return m;
  }, [materiais, matSortBy]);

  const tdStyle = { padding: '11px 14px', borderBottom: '1px solid #1a1a1a', fontSize: 13 };

  if (loading) return <div style={{ padding: '24px 28px' }}><Spin /></div>;

  // ── User Detail View ──
  if (selectedUser && userDetail) {
    const pct = userDetail.materiais_acessados + userDetail.materiais_faltam > 0
      ? Math.round((userDetail.materiais_acessados / (userDetail.materiais_acessados + userDetail.materiais_faltam)) * 100) : 0;

    return (
      <div style={{ padding: '24px 28px', maxWidth: 900 }}>
        <button onClick={() => { setSelectedUser(null); setUserDetail(null); }}
          style={{ ...css.btn, background: '#141414', color: '#999', border: '1px solid #1a1a1a', marginBottom: 20, fontSize: 12 }}>
          ← Voltar ao relatório
        </button>

        {userDetailLoading ? <Spin /> : <>
          {/* User header */}
          <div style={{ ...css.card, padding: '20px 24px', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(232,57,42,0.08)', border: '2px solid rgba(232,57,42,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 16, color: '#E8392A' }}>
                {(userDetail.user.name || userDetail.user.email).substring(0, 2).toUpperCase()}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 700 }}>{userDetail.user.name || userDetail.user.email}</div>
                <div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>{userDetail.user.email} · {userDetail.user.role}{userDetail.user.operadora_name ? ` · ${userDetail.user.operadora_name}` : ''}</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
            <MC label="Total de Acessos" value={userDetail.total_acessos} color="#f5f5f5" />
            <MC label="Materiais Acessados" value={userDetail.materiais_acessados} color="#22c55e" />
            <MC label="Faltam Acessar" value={userDetail.materiais_faltam} color={userDetail.materiais_faltam > 0 ? '#E8392A' : '#22c55e'} />
            <MC label="Progresso" value={`${pct}%`} color={pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A'} />
          </div>

          {/* Progress bar */}
          <div style={{ ...css.card, padding: '16px 20px', marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ fontSize: 12, color: '#555' }}>Progresso no Playbook</span>
              <span style={{ fontSize: 12, fontFamily: "'JetBrains Mono',monospace", color: '#999' }}>{userDetail.materiais_acessados}/{userDetail.materiais_acessados + userDetail.materiais_faltam}</span>
            </div>
            <div style={{ height: 6, background: '#1a1a1a', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 3, background: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A', width: `${pct}%`, transition: 'width 0.5s' }} />
            </div>
          </div>

          {/* Materiais acessados */}
          <Sec icon="✅" title="Materiais Acessados">
            <div style={css.card}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    <th style={css.th}>Material</th>
                    <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Tipo</th>
                    <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Vezes</th>
                    <th style={{ ...css.th, textAlign: 'right', width: 160 }}>Último Acesso</th>
                  </tr>
                </thead>
                <tbody>
                  {userDetail.acessados.map(a => (
                    <tr key={a.id}>
                      <td style={tdStyle}>
                        <div style={{ fontWeight: 500 }}>{a.titulo}</div>
                        {a.descricao && <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>{a.descricao}</div>}
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'center' }}>
                        <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: `${TIPO_COLORS[a.tipo] || '#555'}15`, color: TIPO_COLORS[a.tipo] || '#555' }}>
                          {a.tipo}
                        </span>
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: 16, color: '#f5f5f5' }}>
                        {a.vezes}
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", color: '#555', fontSize: 12 }}>
                        {new Date(a.ultimo_acesso).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' })}
                      </td>
                    </tr>
                  ))}
                  {!userDetail.acessados.length && <tr><td colSpan={4} style={{ padding: 28, color: '#555', textAlign: 'center' }}>Nenhum acesso registrado</td></tr>}
                </tbody>
              </table>
            </div>
          </Sec>
          ...
          <div style={{ marginTop: 20 }}>
            <button onClick={() => setShowFaltam(!showFaltam)}
              style={{ ...css.btn, background: showFaltam ? 'rgba(232,57,42,0.1)' : '#141414', color: showFaltam ? '#E8392A' : '#999', border: `1px solid ${showFaltam ? 'rgba(232,57,42,0.3)' : '#1a1a1a'}`, fontSize: 13, padding: '10px 18px', display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 16 }}>🚫</span>
              Falta acessar ({userDetail.materiais_faltam})
              <span style={{ fontSize: 11, marginLeft: 4 }}>{showFaltam ? '▲' : '▼'}</span>
            </button>

            {showFaltam && userDetail.faltam.length > 0 && (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      <th style={css.th}>#</th>
                      <th style={css.th}>Material</th>
                      <th style={{ ...css.th, textAlign: 'center' }}>Tipo</th>
                    </tr>
                  </thead>
                  <tbody>
                    {userDetail.faltam.map((m, i) => (
                      <tr key={m.id}>
                        <td style={{ ...tdStyle, width: 40, fontFamily: "'JetBrains Mono',monospace", color: '#444' }}>{i + 1}</td>
                        <td style={tdStyle}>
                          <div style={{ fontWeight: 500, color: '#999' }}>{m.titulo}</div>
                          {m.descricao && <div style={{ fontSize: 11, color: '#444', marginTop: 2 }}>{m.descricao}</div>}
                        </td>
                        <td style={{ ...tdStyle, textAlign: 'center' }}>
                          <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: `${TIPO_COLORS[m.tipo] || '#555'}15`, color: TIPO_COLORS[m.tipo] || '#555' }}>
                            {m.tipo}
                          </span>
                        </td>
                      </tr>
    ...
    <div style={{ padding: '24px 28px', maxWidth: 1000 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        {tab === 'tracking' && <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: '#555' }}>Visualização:</span>
          <button onClick={() => setViewMode('cards')} style={{ ...css.btn, background: viewMode === 'cards' ? '#282828' : 'transparent', color: viewMode === 'cards' ? '#f5f5f5' : '#555', padding: '4px 10px', fontSize: 11, border: '1px solid #1a1a1a' }}>■ Cards</button>
          <button onClick={() => setViewMode('lista')} style={{ ...css.btn, background: viewMode === 'lista' ? '#282828' : 'transparent', color: viewMode === 'lista' ? '#f5f5f5' : '#555', padding: '4px 10px', fontSize: 11, border: '1px solid #1a1a1a' }}>☰ Lista</button>
          <div style={{ width: 1, height: 16, background: '#1a1a1a', margin: '0 4px' }} />
          <span style={{ fontSize: 11, color: '#555' }}>Ordenar:</span>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ ...css.input, padding: '4px 8px', fontSize: 11, width: 'auto' }}>
            <option value="acessos">Mais acessos</option>
            <option value="faltam">Mais faltam</option>
      ...
      {tab === 'tracking' && <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
          <MC label="Total de Materiais" value={tracking?.total_materiais || 0} />
          <MC label="Usuários Ativos" value={sortedUsers.length} />
          <MC label="Usuários com 100%" value={sortedUsers.filter(u => u.materiais_acessados >= (tracking?.total_materiais || 0) && tracking?.total_materiais > 0).length} color="#22c55e" />
        </div>
        ...
        {viewMode === 'cards' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
            {sortedUsers.map(u => {
              const total = tracking?.total_materiais || 0;
              const faltam = total - u.materiais_acessados;
              const pct = total > 0 ? Math.round((u.materiais_acessados / total) * 100) : 0;
              return (
                <div key={u.id} onClick={() => setSelectedUser(u.id)}
                  style={{ ...css.card, padding: '18px 20px', cursor: 'pointer', transition: 'all 0.2s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = '#282828'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = '#1a1a1a'; e.currentTarget.style.transform = 'none'; }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(232,57,42,0.08)', border: '1px solid rgba(232,57,42,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 12, color: '#E8392A', flexShrink: 0 }}>
                      {(u.name || u.email).substring(0, 2).toUpperCase()}
                    </div>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: 14, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.name || u.email}</div>
                      <div style={{ fontSize: 11, color: '#555' }}>{u.role}{u.operadora_name ? ` · ${u.operadora_name}` : ''}</div>
                    </div>
                  </div>
                  {/* Progress bar */}
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 11, color: '#555' }}>Progresso</span>
                      <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600, color: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A' }}>{pct}%</span>
                    </div>
                    <div style={{ height: 4, background: '#1a1a1a', borderRadius: 2, overflow: 'hidden' }}>
                      <div style={{ height: '100%', borderRadius: 2, background: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A', width: `${pct}%`, transition: 'width 0.5s' }} />
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 16 }}>
                    <div><div style={{ fontSize: 20, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace" }}>{u.total_acessos}</div><div style={{ fontSize: 10, color: '#555' }}>acessos</div></div>
                    <div><div style={{ fontSize: 20, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: '#22c55e' }}>{u.materiais_acessados}</div><div style={{ fontSize: 10, color: '#555' }}>vistos</div></div>
                    <div><div style={{ fontSize: 20, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: faltam > 0 ? '#E8392A' : '#22c55e' }}>{faltam}</div><div style={{ fontSize: 10, color: '#555' }}>faltam</div></div>
                  </div>
                  {u.ultimo_acesso && <div style={{ marginTop: 10, fontSize: 10, color: '#444', fontFamily: "'JetBrains Mono',monospace" }}>Último: {new Date(u.ultimo_acesso).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</div>}
                </div>
              );
            })}
          </div>
          ...
          <div style={css.card}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={css.th}>Usuário</th>
                  <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Total Acessos</th>
                  <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Vistos</th>
                  <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Faltam</th>
                  <th style={{ ...css.th, textAlign: 'center', width: 120 }}>Progresso</th>
                  <th style={{ ...css.th, textAlign: 'right', width: 140 }}>Último Acesso</th>
                </tr>
              </thead>
              <tbody>
                {sortedUsers.map(u => {
                    ...
                    <tr key={u.id} onClick={() => setSelectedUser(u.id)} style={{ cursor: 'pointer' }}
                      ...
                      <td style={tdStyle}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(232,57,42,0.08)', border: '1px solid rgba(232,57,42,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 10, color: '#E8392A', flexShrink: 0 }}>
                            {(u.name || u.email).substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <div style={{ fontWeight: 500, fontSize: 13 }}>{u.name || u.email}</div>
                            <div style={{ fontSize: 11, color: '#555' }}>{u.email}</div>
                          </div>
                        </div>
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: 15 }}>{u.total_acessos}</td>
                      <td style={{ ...tdStyle, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", color: '#22c55e', fontWeight: 600 }}>{u.materiais_acessados}</td>
                      <td style={{ ...tdStyle, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", color: faltam > 0 ? '#E8392A' : '#22c55e', fontWeight: 600 }}>{faltam}</td>
                      <td style={{ ...tdStyle, textAlign: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ flex: 1, height: 4, background: '#1a1a1a', borderRadius: 2, overflow: 'hidden' }}>
                            <div style={{ height: '100%', borderRadius: 2, background: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A', width: `${pct}%` }} />
                          </div>
                          <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600, color: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A', minWidth: 32 }}>{pct}%</span>
                        </div>
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", color: '#555', fontSize: 11 }}>
                        {u.ultimo_acesso ? new Date(u.ultimo_acesso).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'}
                      </td>
                    </tr>
                  );
                })}
                {!sortedUsers.length && <tr><td colSpan={6} style={{ padding: 28, color: '#555', textAlign: 'center' }}>Nenhum usuário ativo</td></tr>}
              </tbody>
            </table>
          </div>
        )}
      ...
      {tab === 'meu-progresso' && meuProgresso && <>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
          ...
          <MC label="Faltam Acessar" value={meuProgresso.faltam?.length || 0} color={meuProgresso.faltam?.length > 0 ? '#E8392A' : '#22c55e'} />
        </div>
        {/* Progress bar */}
        {(() => { const total = (meuProgresso.acessados?.length || 0) + (meuProgresso.faltam?.length || 0); const pct = total > 0 ? Math.round(((meuProgresso.acessados?.length || 0) / total) * 100) : 0; return (
          <div style={{ ...css.card, padding: '16px 20px', marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ fontSize: 12, color: '#555' }}>Progresso Geral</span>
              <span style={{ fontSize: 12, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600, color: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A' }}>{pct}%</span>
            </div>
            <div style={{ height: 6, background: '#1a1a1a', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 3, background: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A', width: `${pct}%`, transition: 'width 0.5s' }} />
            </div>
        ...
        <Sec icon="✅" title="Materiais que Acessei">
          <div style={css.card}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>
                <th style={css.th}>Material</th>
                <th style={{ ...css.th, textAlign: 'center', width: 80 }}>Tipo</th>
                <th style={{ ...css.th, textAlign: 'center', width: 80 }}>Vezes</th>
                <th style={{ ...css.th, textAlign: 'right', width: 140 }}>Último</th>
              </tr></thead>
              <tbody>
                {(meuProgresso.acessados || []).map(a => (
                  <tr key={a.id}>
                    <td style={tdStyle}><div style={{ fontWeight: 500 }}>{a.titulo}</div>{a.descricao && <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>{a.descricao}</div>}</td>
                    <td style={{ ...tdStyle, textAlign: 'center' }}><span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: `${TIPO_COLORS[a.tipo] || '#555'}15`, color: TIPO_COLORS[a.tipo] || '#555' }}>{a.tipo}</span></td>
                    <td style={{ ...tdStyle, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: 16 }}>{a.vezes}</td>
                    <td style={{ ...tdStyle, textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", color: '#555', fontSize: 11 }}>{new Date(a.ultimo_acesso).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</td>
                  </tr>
                ))}
                {!meuProgresso.acessados?.length && <tr><td colSpan={4} style={{ padding: 28, color: '#555', textAlign: 'center' }}>Nenhum acesso registrado ainda</td></tr>}
              </tbody>
            </table>
          </div>
        </Sec>
        <div style={{ marginTop: 16 }}>
          <button onClick={() => setShowFaltam(!showFaltam)} style={{ ...css.btn, background: showFaltam ? 'rgba(232,57,42,0.1)' : '#141414', color: showFaltam ? '#E8392A' : '#999', border: `1px solid ${showFaltam ? 'rgba(232,57,42,0.3)' : '#1a1a1a'}`, fontSize: 13, padding: '10px 18px' }}>
            🚫 Falta acessar ({meuProgresso.faltam?.length || 0}) {showFaltam ? '▲' : '▼'}
          </button>
          {showFaltam && (meuProgresso.faltam || []).length > 0 && (
            <div style={{ ...css.card, marginTop: 12 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr><th style={{ ...css.th, width: 40 }}>#</th><th style={css.th}>Material</th><th style={{ ...css.th, textAlign: 'center' }}>Tipo</th></tr></thead>
                <tbody>
                  {meuProgresso.faltam.map((m, i) => (
                    <tr key={m.id}>
                      <td style={{ ...tdStyle, fontFamily: "'JetBrains Mono',monospace", color: '#444' }}>{i + 1}</td>
                      <td style={tdStyle}><div style={{ fontWeight: 500, color: '#999' }}>{m.titulo}</div></td>
                      <td style={{ ...tdStyle, textAlign: 'center' }}><span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: `${TIPO_COLORS[m.tipo] || '#555'}15`, color: TIPO_COLORS[m.tipo] || '#555' }}>{m.tipo}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {tab === 'meus-cursos' && <>
        <div style={css.card}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              <th style={{ ...css.th, width: 50 }}>#</th>
              <th style={css.th}>Material</th>
              <th style={{ ...css.th, textAlign: 'center', width: 80 }}>Tipo</th>
              <th style={{ ...css.th, textAlign: 'center', width: 80 }}>Ação</th>
            </tr></thead>
            <tbody>
              {materiais.map((m, i) => {
                const acessou = meuProgresso?.acessados?.find(a => a.id === m.id);
                return (
                  <tr key={m.id}>
                    <td style={{ ...tdStyle, fontFamily: "'JetBrains Mono',monospace", color: '#444' }}>{i + 1}</td>
                    <td style={tdStyle}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {acessou ? <span style={{ color: '#22c55e', fontSize: 14 }}>✓</span> : <span style={{ color: '#333', fontSize: 14 }}>○</span>}
                        <div>
                          <div style={{ fontWeight: 500 }}>{m.titulo}</div>
                          {m.descricao && <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>{m.descricao}</div>}
                        </div>
                      </div>
                    </td>
                    <td style={{ ...tdStyle, textAlign: 'center' }}><span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: `${TIPO_COLORS[m.tipo] || '#555'}15`, color: TIPO_COLORS[m.tipo] || '#555' }}>{m.tipo}</span></td>
                    <td style={{ ...tdStyle, textAlign: 'center' }}>
                      {m.url ? <a href={m.url} target="_blank" rel="noopener" onClick={() => apiPlaybook.logAccess(m.id)} style={{ ...css.btn, background: '#E8392A', color: '#fff', fontSize: 11, padding: '4px 12px', textDecoration: 'none' }}>Acessar</a>
                        : <button onClick={() => apiPlaybook.logAccess(m.id).then(loadData)} style={{ ...css.btn, background: '#141414', color: '#999', border: '1px solid #1a1a1a', fontSize: 11, padding: '4px 12px' }}>Marcar</button>}
                    </td>
                  </tr>
                );
              })}
      ...
      {tab === 'minhas-trilhas' && <>
        ...
        {minhasTrilhas.map(tr => {
          const pct = tr.total > 0 ? Math.round((tr.vistos / tr.total) * 100) : 0;
          const isExpanded = expandedTrilha === tr.id;
          return (
            <div key={tr.id} style={{ ...css.card, marginBottom: 12, padding: 0, overflow: 'hidden' }}>
              <div onClick={() => setExpandedTrilha(isExpanded ? null : tr.id)} style={{ padding: '16px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                onMouseEnter={e => e.currentTarget.style.background = '#0e0e0e'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 15 }}>🎯 {tr.nome}</div>
                  {tr.descricao && <div style={{ fontSize: 12, color: '#555', marginTop: 3 }}>{tr.descricao}</div>}
                  <div style={{ fontSize: 11, color: '#444', marginTop: 4 }}>Criado por {tr.criado_por}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 11, color: '#555' }}>{tr.vistos}/{tr.total} materiais</div>
                    <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: pct >= 80 ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#E8392A' }}>{pct}%</div>
                  </div>
              ...
              {isExpanded && (
                <div style={{ borderTop: '1px solid #1a1a1a', padding: '12px 20px' }}>
                  {tr.materiais.map((m, i) => (
                    <div key={m.material_id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < tr.materiais.length - 1 ? '1px solid #111' : 'none' }}>
                      <span style={{ color: m.vezes > 0 ? '#22c55e' : '#333', fontSize: 14, width: 20 }}>{m.vezes > 0 ? '✓' : '○'}</span>
                      <div style={{ flex: 1 }}>
                        <span style={{ fontSize: 13, fontWeight: 500, color: m.vezes > 0 ? '#f5f5f5' : '#888' }}>{m.titulo}</span>
                        <span style={{ fontSize: 11, padding: '1px 6px', borderRadius: 3, marginLeft: 8, background: `${TIPO_COLORS[m.tipo] || '#555'}15`, color: TIPO_COLORS[m.tipo] || '#555' }}>{m.tipo}</span>
                      </div>
                      {m.vezes > 0 && <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono',monospace", color: '#555' }}>{m.vezes}x</span>}
                      {m.url && <a href={m.url} target="_blank" rel="noopener" onClick={() => apiPlaybook.logAccess(m.material_id)} style={{ fontSize: 11, color: '#E8392A', textDecoration: 'none' }}>Abrir →</a>}
                    </div>
                  ))}
                </div>
              )}
      ...
      {/* ── TRILHAS TAB (master/gestor) ── */}
      {tab === 'trilhas' && canManageTrilhas && <>
        {/* Trilha detail view */}
        {selectedTrilha && trilhaDetail ? (
          <div>
            <button onClick={() => { setSelectedTrilha(null); setTrilhaDetail(null); }} style={{ ...css.btn, background: '#141414', color: '#999', border: '1px solid #1a1a1a', marginBottom: 20, fontSize: 12 }}>← Voltar</button>
            <div style={{ ...css.card, padding: '20px 24px', marginBottom: 20 }}>
              <div style={{ fontSize: 18, fontWeight: 700 }}>🎯 {trilhaDetail.trilha.nome}</div>
              {trilhaDetail.trilha.descricao && <div style={{ fontSize: 13, color: '#555', marginTop: 6 }}>{trilhaDetail.trilha.descricao}</div>}
              <div style={{ fontSize: 11, color: '#444', marginTop: 6 }}>Criado por {trilhaDetail.trilha.criado_por}</div>
            </div>

            {/* Materiais da trilha */}
            <Sec icon="📚" title={`Materiais (${trilhaDetail.materiais.length})`}>
              <div style={css.card}>
                {trilhaDetail.materiais.map(m => (
                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid #1a1a1a' }}>
                    <div><span style={{ fontWeight: 500 }}>{m.titulo}</span> <span style={{ fontSize: 11, padding: '1px 6px', borderRadius: 3, marginLeft: 6, background: `${TIPO_COLORS[m.tipo] || '#555'}15`, color: TIPO_COLORS[m.tipo] || '#555' }}>{m.tipo}</span></div>
                    <button onClick={() => removeMatFromTrilha(selectedTrilha, m.id)} style={{ ...css.btn, background: 'rgba(232,57,42,0.08)', color: '#E8392A', fontSize: 11, padding: '3px 8px' }}>✕</button>
                  </div>
                ))}
                {!trilhaDetail.materiais.length && <div style={{ padding: 20, textAlign: 'center', color: '#555' }}>Nenhum material</div>}
              </div>
              {/* Add material dropdown */}
              <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                <select id="addMatToTrilha" style={{ ...css.input, flex: 1, fontSize: 12 }}>
                  <option value="">+ Adicionar material...</option>
                  {materiais.filter(m => !trilhaDetail.materiais.find(tm => tm.id === m.id)).map(m => (
                    <option key={m.id} value={m.id}>{m.titulo}</option>
                  ))}
                </select>
                <button onClick={() => { const sel = document.getElementById('addMatToTrilha'); if (sel.value) addMatToTrilha(selectedTrilha, Number(sel.value)); }}
                  style={{ ...css.btn, background: '#E8392A', color: '#fff', fontSize: 12 }}>Adicionar</button>
              </div>
            </Sec>

            {/* Usuários da trilha */}
            <Sec icon="👥" title={`Usuários (${trilhaDetail.usuarios.length})`}>
              <div style={css.card}>
                {trilhaDetail.usuarios.map(u => (
                  <div key={u.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid #1a1a1a' }}>
                    <div><span style={{ fontWeight: 500 }}>{u.name || u.email}</span> <span style={{ fontSize: 11, color: '#555', marginLeft: 8 }}>{u.email}</span></div>
                    <button onClick={() => removeUserFromTrilha(selectedTrilha, u.id)} style={{ ...css.btn, background: 'rgba(232,57,42,0.08)', color: '#E8392A', fontSize: 11, padding: '3px 8px' }}>✕</button>
                  </div>
                ))}
                {!trilhaDetail.usuarios.length && <div style={{ padding: 20, textAlign: 'center', color: '#555' }}>Nenhum usuário atribuído</div>}
              </div>
              <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                <select id="addUserToTrilha" style={{ ...css.input, flex: 1, fontSize: 12 }}>
                  <option value="">+ Atribuir usuário...</option>
                  {availableUsers.filter(u => u.role !== 'master' && !trilhaDetail.usuarios.find(tu => tu.id === u.id)).map(u => (
                    <option key={u.id} value={u.id}>{u.name || u.email}</option>
                  ))}
                </select>
                <button onClick={() => { const sel = document.getElementById('addUserToTrilha'); if (sel.value) addUserToTrilha(selectedTrilha, Number(sel.value)); }}
                  style={{ ...css.btn, background: '#E8392A', color: '#fff', fontSize: 12 }}>Atribuir</button>
              </div>
            </Sec>
          </div>
        ) : (
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 14, color: '#555' }}>{trilhas.length} trilha{trilhas.length !== 1 ? 's' : ''}</div>
              <button onClick={() => { resetTrilhaForm(); setShowTrilhaForm(true); }} style={{ ...css.btn, background: '#E8392A', color: '#fff', fontSize: 12 }}>+ Nova Trilha</button>
            </div>

            {showTrilhaForm && (
              <div style={{ ...css.card, padding: '20px 24px', marginBottom: 16, borderColor: 'rgba(232,57,42,0.3)' }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>{editingTrilha ? 'Editar Trilha' : 'Nova Trilha'}</div>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Nome *</label>
                  <input value={trilhaNome} onChange={e => setTrilhaNome(e.target.value)} style={{ ...css.input, width: '100%' }} placeholder="Ex: Onboarding, Treinamento Avançado..." />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Descrição</label>
                  <input value={trilhaDesc} onChange={e => setTrilhaDesc(e.target.value)} style={{ ...css.input, width: '100%' }} placeholder="Descrição da trilha" />
                </div>
                {!editingTrilha && <>
                  <div style={{ marginBottom: 12 }}>
                    <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Materiais (selecione vários)</label>
                    <div style={{ maxHeight: 150, overflow: 'auto', border: '1px solid #1a1a1a', borderRadius: 8, padding: 8 }}>
                      {materiais.map(m => (
                        <label key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 12, color: '#ccc', cursor: 'pointer' }}>
                          <input type="checkbox" checked={trilhaMatIds.includes(m.id)} onChange={e => setTrilhaMatIds(prev => e.target.checked ? [...prev, m.id] : prev.filter(id => id !== m.id))} />
                          {m.titulo} <span style={{ fontSize: 10, color: '#555' }}>({m.tipo})</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Atribuir a usuários</label>
                    <div style={{ maxHeight: 150, overflow: 'auto', border: '1px solid #1a1a1a', borderRadius: 8, padding: 8 }}>
                      {availableUsers.filter(u => u.role !== 'master').map(u => (
                        <label key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 12, color: '#ccc', cursor: 'pointer' }}>
                          <input type="checkbox" checked={trilhaUserIds.includes(u.id)} onChange={e => setTrilhaUserIds(prev => e.target.checked ? [...prev, u.id] : prev.filter(id => id !== u.id))} />
                          {u.name || u.email} <span style={{ fontSize: 10, color: '#555' }}>({u.role})</span>
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={saveTrilha} disabled={trilhaSaving || !trilhaNome.trim()} style={{ ...css.btn, background: '#E8392A', color: '#fff', opacity: trilhaSaving || !trilhaNome.trim() ? 0.5 : 1 }}>
                    {trilhaSaving ? 'Salvando...' : editingTrilha ? 'Salvar' : 'Criar Trilha'}
                  </button>
                  <button onClick={resetTrilhaForm} style={{ ...css.btn, background: '#141414', color: '#999', border: '1px solid #1a1a1a' }}>Cancelar</button>
                </div>
              </div>
            )}

            {trilhas.map(tr => (
              <div key={tr.id} style={{ ...css.card, padding: '16px 20px', marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ cursor: 'pointer', flex: 1 }} onClick={() => { setSelectedTrilha(tr.id); loadTrilhaDetail(tr.id); }}>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>🎯 {tr.nome}</div>
                  {tr.descricao && <div style={{ fontSize: 12, color: '#555', marginTop: 3 }}>{tr.descricao}</div>}
                  <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
                    <span style={{ fontSize: 11, color: '#555' }}>📚 {tr.total_materiais} materiais</span>
                    <span style={{ fontSize: 11, color: '#555' }}>👥 {tr.total_usuarios} usuários</span>
                    <span style={{ fontSize: 11, color: '#444' }}>por {tr.criado_por}</span>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 4 }}>
                  <button onClick={() => { setEditingTrilha(tr.id); setTrilhaNome(tr.nome); setTrilhaDesc(tr.descricao || ''); setShowTrilhaForm(true); }}
                    style={{ ...css.btn, background: 'rgba(255,255,255,0.04)', color: '#999', fontSize: 11, padding: '4px 10px', border: '1px solid #1a1a1a' }}>✏️</button>
                  <button onClick={() => { setSelectedTrilha(tr.id); loadTrilhaDetail(tr.id); }}
                    style={{ ...css.btn, background: 'rgba(255,255,255,0.04)', color: '#999', fontSize: 11, padding: '4px 10px', border: '1px solid #1a1a1a' }}>Abrir</button>
                  <button onClick={() => deleteTrilha(tr.id)}
                    style={{ ...css.btn, background: 'rgba(232,57,42,0.08)', color: '#E8392A', fontSize: 11, padding: '4px 10px' }}>🗑</button>
                </div>
              </div>
            ))}
            {!trilhas.length && !showTrilhaForm && <div style={{ ...css.card, padding: 40, textAlign: 'center' }}>
              <div style={{ fontSize: 32, opacity: .3, marginBottom: 8 }}>🎯</div>
              <div style={{ color: '#555', fontSize: 14 }}>Nenhuma trilha criada. Clique em "+ Nova Trilha".</div>
            </div>}
          </div>
        )}
      </>}

      {tab === 'materiais' && canManageContent && <>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: '#555' }}>Ordenar:</span>
            <select value={matSortBy} onChange={e => setMatSortBy(e.target.value)} style={{ ...css.input, padding: '4px 8px', fontSize: 11, width: 'auto' }}>
              <option value="ordem">Ordem manual</option>
              <option value="data">Data de inserção</option>
              <option value="titulo">Título A-Z</option>
            </select>
          </div>
          <button onClick={() => { resetForm(); setShowForm(true); setMatOrdem(materiais.length + 1); }}
            style={{ ...css.btn, background: '#E8392A', color: '#fff', fontSize: 12 }}>
            + Adicionar Material
          </button>
        </div>
        ...
        {showForm && (
          <div style={{ ...css.card, padding: '20px 24px', marginBottom: 16, borderColor: 'rgba(232,57,42,0.3)' }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>{editingMat ? 'Editar Material' : 'Novo Material'}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Título *</label>
                <input value={matTitulo} onChange={e => setMatTitulo(e.target.value)} style={{ ...css.input, width: '100%' }} placeholder="Nome do material" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px', gap: 8 }}>
                <div>
                  <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Tipo</label>
                  <select value={matTipo} onChange={e => setMatTipo(e.target.value)} style={{ ...css.input, width: '100%' }}>
                    {Object.entries(TIPO_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Ordem</label>
                  <input type="number" value={matOrdem} onChange={e => setMatOrdem(Number(e.target.value))} style={{ ...css.input, width: '100%' }} min={0} />
                </div>
              </div>
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Descrição</label>
              <input value={matDesc} onChange={e => setMatDesc(e.target.value)} style={{ ...css.input, width: '100%' }} placeholder="Descrição breve do material" />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>URL (link do material)</label>
              <input value={matUrl} onChange={e => setMatUrl(e.target.value)} style={{ ...css.input, width: '100%' }} placeholder="https://..." />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={saveMaterial} disabled={matSaving || !matTitulo.trim()}
                style={{ ...css.btn, background: '#E8392A', color: '#fff', opacity: matSaving || !matTitulo.trim() ? 0.5 : 1 }}>
                {matSaving ? 'Salvando...' : editingMat ? 'Salvar Alterações' : 'Criar Material'}
              </button>
              <button onClick={resetForm} style={{ ...css.btn, background: '#141414', color: '#999', border: '1px solid #1a1a1a' }}>Cancelar</button>
            </div>
          </div>
        )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                ...
                <th style={css.th}>Material</th>
                <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Tipo</th>
                <th style={{ ...css.th, textAlign: 'center', width: 120 }}>Adicionado em</th>
                <th style={{ ...css.th, textAlign: 'center', width: 100 }}>Ações</th>
              </tr>
            </thead>
            <tbody>
              {sortedMateriais.map((m, i) => (
                <tr key={m.id}>
                  <td style={{ ...tdStyle, fontFamily: "'JetBrains Mono',monospace", color: '#444', fontSize: 12 }}>{m.ordem || i + 1}</td>
                  <td style={tdStyle}>
                    <div style={{ fontWeight: 500 }}>{m.titulo}</div>
                    {m.descricao && <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>{m.descricao}</div>}
                    {m.url && <div style={{ fontSize: 11, color: '#3b82f6', marginTop: 2, wordBreak: 'break-all' }}>{m.url}</div>}
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'center' }}>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: `${TIPO_COLORS[m.tipo] || '#555'}15`, color: TIPO_COLORS[m.tipo] || '#555' }}>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace", color: '#555', fontSize: 11 }}>
                    {new Date(m.created_at).toLocaleDateString('pt-BR')}
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'center' }}>
                    <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
                      <button onClick={() => openEditForm(m)} style={{ ...css.btn, background: 'rgba(255,255,255,0.04)', color: '#999', fontSize: 11, padding: '3px 8px', border: '1px solid #1a1a1a' }}>✏️</button>
                      <button onClick={() => deleteMaterial(m.id)} style={{ ...css.btn, background: 'rgba(232,57,42,0.08)', color: '#E8392A', fontSize: 11, padding: '3px 8px' }}>🗑</button>
                    </div>
                  </td>
                </tr>
              ))}
              {!sortedMateriais.length && <tr><td colSpan={5} style={{ padding: 28, color: '#555', textAlign: 'center' }}>Nenhum material cadastrado. Clique em "+ Adicionar Material".</td></tr>}
            </tbody>
          </table>
        </div>
      </>}
  ...
  return <div style={{ position: 'relative', zIndex: 1 }}>
    ...
    <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 28px', borderBottom: '1px solid #1a1a1a', backdropFilter: 'blur(20px)', background: 'rgba(5,5,5,0.85)', position: 'sticky', top: 0, zIndex: 50 }}>
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        {(canManage
          ? (isMaster ? ['operadora', 'diretoria', 'playbook', 'config', 'usuarios'] : ['operadora', 'diretoria', 'playbook', 'config'])
          : ['operadora', 'playbook']
        ).map(v => (
          <button key={v} onClick={() => setView(v)} style={{ ...css.btn, background: view === v ? '#E8392A' : '#141414', color: view === v ? '#fff' : '#999', border: view === v ? 'none' : '1px solid #1a1a1a', textTransform: 'capitalize' }}>
            {v === 'config' ? '⚙ Config' : v === 'usuarios' ? '👥 Usuários' : v === 'playbook' ? '📚 Playbook' : v}
          </button>
        ))}
    ...
    {/* PLAYBOOK — all users */}
    {view === 'playbook' && <PlaybookTab isMaster={isMaster} isGestor={isAdmin} authUser={authUser} />}
